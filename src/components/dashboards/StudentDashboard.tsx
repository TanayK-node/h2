...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout }) => {
          ...
          <div key={subject.id} className="bg-white rounded-lg shadow-md p-4">
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <span className={`text-sm font-medium ${
                subject.attendance < 75 ? 'text-red-600' : 
                subject.attendance < 85 ? 'text-yellow-600' : 
                'text-green-600'
              }`}>
                {subject.attendance}%
              </span>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
              <div
                className={`rounded-full h-2 ${
                  subject.attendance < 75 ? 'bg-red-500' : 
                  subject.attendance < 85 ? 'bg-yellow-500' : 
                  'bg-green-500'
                }`}
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-10 bg-black bg-opacity-50" onClick={() => setMobileMenuOpen(false)}>
          <div className="absolute top-14 left-0 bottom-0 w-64 bg-primary-900 shadow-lg" onClick={e => e.stopPropagation()}>
